...
These files are **deliberately not committed by this project's authors**. NRL
club logos are registered trademarks of their clubs; because GenPicks presents
as a product (Pro tier, checkout flow), shipping club marks could imply an
affiliation or endorsement that does not exist. Supply only files you hold the
rights to use, and check `../../..//docs` and the site footer stay accurate
about affiliation.
...
- Marks render at 18×18 and are `alt=""`: the team name always sits beside
  them, so describing the logo would announce the club twice.
